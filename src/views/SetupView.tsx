...
const INSTALL_STEPS: Step[] = [
  { text: t.stepInstallCli, command: 'brew tap ScalableCapital/tap' },
  { text: '', command: 'brew trust --formula ScalableCapital/tap/scalable-cli' },
  { text: '', command: 'brew install scalable-cli' },
  { text: t.stepEnableCli },
  // --local-read-only lässt schon die offizielle Binary Mutationen verweigern —
  // eine zweite Schicht unter der Allowlist dieser App.
  { text: t.stepLogin, command: 'sc login --local-read-only' },
  { text: t.stepVerify, command: 'sc whoami' },
]
...
export function SetupView({ error, width, height }: SetupViewProps): React.ReactElement {
  ...
  const intro: Line[] = wrap(missingBinary ? t.setupMissingIntro : t.setupLoginIntro, inner).map(
    (text, i) => ({ key: `intro-${i}`, segments: [{ text, color: theme.muted }] }),
  )
  ...
  const demoHint: Line =
    inner >= 84
      ? {
          key: 'demo',
          segments: [
            { text: t.setupDemoLongPrefix, color: theme.muted },
            { text: 'sctui --demo', color: theme.accent },
            { text: t.setupDemoLongSuffix, color: theme.muted },
          ],
        }
      : {
          key: 'demo',
          segments: [
            { text: t.setupDemoShortPrefix, color: theme.muted },
            { text: 'sctui --demo', color: theme.accent },
            { text: t.setupDemoShortSuffix, color: theme.muted },
          ],
        }

  const footer: Line[] = [
    { key: 'error', segments: [{ text: fit(`${t.setupErrorLabel}${error.message}`), color: theme.dim }] },
    blank('gap-2'),
    demoHint,
    { key: 'keys', segments: [{ text: t.setupKeys, color: theme.dim }] },
  ]
  ...
  return (
    <Panel title={missingBinary ? t.setupMissingTitle : t.setupLoginTitle} width={width} height={height}>
